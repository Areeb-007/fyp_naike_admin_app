...
class DioServices {
  ...
  final String mujtabaApiUrlForBC = 'http://172.20.10.2:3000/';
  final String mujtabaApiUrlForBCHome = 'http://192.168.10.22:3000/';
  final String GET_NEED_COUNT = 'v1/need/count';
  final String GET_DONATION_COUNT = 'v1/donation/count';
  final String Add_CATEGORY = 'v1/category';
  final String GET_CATEGORY = 'v1/category';
  //-------------------------------- Constructor ----------------------------------//
  ...
  initializeInterceptors() {
    _dio.interceptors.add(InterceptorsWrapper(
      ...
      final String? action = prefs.getString(StringsRes.tokenInSharedPrefs);
  ...
  Future<bool> loginUser(BuildContext context, bool rememberMe) async {
    ...
    try {
      ...
      await prefs.setString(StringsRes.tokenInSharedPrefs, _token);
...
//-------------------------------- Get the User Data ----------------------------------//
  ...
  Future<List<Categories>> getTheCategoriesList() async {
    ...
    try {
      Response response = await _dio.get(mujtabaApiUrlForBCHome + GET_CATEGORY);
      print(response.data);
  ...
  Future<int> getTheNeedsCount() async {
    int count = 0;
    try {
      Response response =
          await _dio.get(mujtabaApiUrlForBCHome + GET_NEED_COUNT);
      print(response.data);
  ...
  Future<int> getTheDonationsCount() async {
    int count = 0;
    try {
      Response response =
          await _dio.get(mujtabaApiUrlForBCHome + GET_DONATION_COUNT);
      print(response.data);
      count = (response.data['Data']['data']['count']);
    } on DioError catch (_) {
      print('Landed on the Get Categories catch part');
...
// //-------------------------------- Add the Category on Block CHain ----------------------------------//
  Future<bool> addCategory(String name, BuildContext context) async {
    _context = context;
    try {
      // var formData =
      //     FormData.fromMap({'email': user.email, 'password': user.password});
      ...
      print(data);
      Response response =
          await _dio.post(mujtabaApiUrlForBCHome + Add_CATEGORY, data: data);
...
// //-------------------------------- Add the Category on Block CHain ----------------------------------//
  // Future<bool> addCategoriesOnBC(String name) async {
